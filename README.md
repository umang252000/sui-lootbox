# 📦 Sui Loot Box System with On-Chain Randomness
...
This project was built for the Sui Move Smart Contract Hackathon – Gaming Track.
...
- On-chain randomness using Sui’s validator-driven randomness beacon  
...
- 🎁 Purchase unopened loot boxes using SUI  
- 🎲 Open loot boxes using secure on-chain randomness  
- Mint unique NFT game items  
- ⭐ Multiple rarity tiers (Common → Legendary)  
- 🔁 Transfer NFTs between users  
- 🔥 Burn unwanted NFTs  
- Admin-controlled rarity balancing  
- 💎 Pity system guaranteeing a Legendary after 30 non-legendary opens (bonus)  

---

## Architecture

The system follows Sui’s object-oriented model:

```bash

User
├─ buys LootBox (owned object)
├─ opens LootBox (entry function)
│ ├─ randomness generated on-chain
│ ├─ rarity decided
│ ├─ GameItem NFT minted
│ ├─ LootBox burned
│ └─ Event emitted
└─ owns / transfers / burns GameItem
...
Each loot box open generates a random number between `0–99`.
...
The contract uses Sui’s native randomness module:
